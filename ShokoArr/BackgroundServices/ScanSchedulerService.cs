...
public class ScanSchedulerService(ISystemService systemService, MissingEpisodeScanner scanner, ScanCacheStore cacheStore) : BackgroundService
{
    ...
    {
        s_logger.Info("ShokoArr scan scheduler waiting for Shoko Server to reach 'Started' state...");
        while (!systemService.IsStarted && !stoppingToken.IsCancellationRequested)
            await Task.Delay(TimeSpan.FromSeconds(1), stoppingToken).ConfigureAwait(false);
        if (stoppingToken.IsCancellationRequested)
            return;

        s_logger.Info("ShokoArr scan scheduler started.");
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                {
                    s_logger.Info("ShokoArr: running scheduled missing-episode scan.");
            ...
            {
                s_logger.Warn(ex, "ShokoArr: scheduled scan loop error, retrying in 1 minute.");
