    ...
    public async Task NotifyAsync(SonarrSettings settings, string message, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
                s_logger.Warn("ShokoArr: notification webhook returned {StatusCode}: {Message}", (int)response.StatusCode, SanitizeForLog(message));
        }
        catch (Exception ex)
        {
            s_logger.Warn(ex, "ShokoArr: failed to post notification: {Message}", SanitizeForLog(message));
        }
    }
